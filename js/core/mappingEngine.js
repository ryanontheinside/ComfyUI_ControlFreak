...
export class MappingEngine {
    constructor(eventBus) {
        ...
        // Wait for the app to be ready before loading mappings
        setTimeout(() => this.loadMappings(), 1000);
    }
    ...
    importMappingsFromWorkflow(workflowMappings) {
        const importedData = this.persistenceManager.importMappingsFromWorkflow({
            profile: this.activeProfile,
            mappings: workflowMappings
        });
        
        if (importedData && Array.isArray(importedData.mappings)) {
            importedData.mappings.forEach(mapping => {
                // Add if not already exists with same ID
                if (!this.mappings.some(m => m.id === mapping.id)) {
                    this.mappings.push(mapping);
                }
            });
            
            // Emit mappings imported event
            this.eventBus.emit('mappings:imported', importedData.mappings);
        }
    }
    ...
    /**
     * Gets all mapping objects for a specific widget on a specific node in the active profile.
     * @param {number|string} nodeId The ID of the node.
     * @param {string} widgetName The name of the widget.
     * @returns {Array<object>} An array of mapping objects found.
     */
    getMappingsForWidget(nodeId, widgetName) {
        if (!this.mappings || this.mappings.length === 0) {
            return [];
        }
        const activeProfile = this.getActiveProfile();
