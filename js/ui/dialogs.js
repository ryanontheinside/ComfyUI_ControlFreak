...
import { addDialogStyles } from './styles/index.js';

// Ensure dialog styles are loaded
addDialogStyles();
...
function showLearningDialog(learningTarget, callbacks = {}) {
    ...
    mapButton.onclick = () => {
        ...
        // Log what we find in the state manager
        // console.log("[ControlFreak Debug] Map button clicked. Checking learningState.selectedControl:", learningState ? learningState.selectedControl : 'LearningState N/A');
    ...
    // Set up global event listener for controller input detection
    const unsubscribe = eventBus.on('controller:input', (controlInput) => {
        // Import the handler dynamically to prevent circular dependencies
        import('../handlers/controllerEventHandlers.js').then(({ showDetectedInput }) => {
    ...
    // Store unsubscribe function on dialog to clean up when closed
